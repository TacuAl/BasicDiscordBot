...
class Player(commands.Cog):
    ...
    async def play_song(self, ctx, url):
        ...
        with youtube_dl.YoutubeDL({'format' : "bestaudio"}) as ydl:
            ...
            url1 = info['formats'][0]['url']
            source = await discord.FFmpegOpusAudio.from_probe(url1,**FFMPEG_OPTIONS)
            ctx.voice_client.play(source , after=lambda error: self.bot.loop.create_task(self.check_queue(ctx)))

    @commands.command()
    async def leave(self, ctx):
        if ctx.voice_client is not None:
            while len(self.song_queue[ctx.guild.id]) > 0 :
              self.song_queue[ctx.guild.id].pop(0)
    ...
    async def join(self, ctx):
        ...
        if ctx.voice_client is not None and ctx.voice_client is not ctx.author.voice:
            return await ctx.send("I am currently in another voice channel , please wait!")
